...
3. Cuando esté listo, ve a **Project Settings** (icono engranaje) → **API**.
   - Anota **Project URL**.
...
1. Crea un repo en [github.com](https://github.com) (ej. `pm-ai-engine`). No inicialices con README si ya tienes código local.
...
6. **Save** y haz un **Redeploy** (Deployments → ⋮ en el último deploy → Redeploy) para que tome las variables.
...
| 2 | Poner URL y keys de Supabase | `.env` local |
| 3 | Probar subida de PDF | `npm run dev` → localhost |
| 4 | Subir código | GitHub |
| 5 | Conectar repo y configurar env | Vercel |

Después de esto: **código en GitHub**, **app en Vercel**, **datos y PDFs en Supabase**. Cada push a `main` puede desplegar automáticamente en Vercel si dejaste la opción por defecto.
